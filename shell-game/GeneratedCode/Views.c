...
static const XPoint _Const0002 = { 1, 1 };
...
void ViewsBevel__Init( ViewsBevel _this, XObject aLink, XHandle aArg )
{
  ...
  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_.XObject._.GCT = EW_CLASS_GCT( ViewsBevel );

  /* Setup the VMT pointer */
  _this->_.VMT = EW_CLASS( ViewsBevel );

  /* ... and initialize objects, variables, properties, etc. */
  _this->Width = 1;
  _this->ColorL = _Const0001;
  ...
  aOffset, XInt32 aOpacity, XBool aBlend )
{
  ...
  if ( aOpacity < 256 )
  {
    ct.Alpha = (XUInt8)(( aOpacity * 255 ) >> 8 );
    cr.Alpha = (XUInt8)(( aOpacity * cr.Alpha ) >> 8 );
    cl.Alpha = (XUInt8)(( aOpacity * cl.Alpha ) >> 8 );
    cb.Alpha = (XUInt8)(( aOpacity * 255 ) >> 8 );
  }

  r.Point2 = EwMovePointNeg( r.Point2, _Const0002 );
...
EW_DEFINE_CLASS( ViewsBevel, CoreRectView, _.VMT, _.VMT, _.VMT, _.VMT, _.VMT, _.VMT, 
                 "Views::Bevel" )
  CoreRectView_initLayoutContext,
  CoreView_GetRoot,
  ViewsBevel_Draw,
      ...
      != 0 ))
  {
    ...
    if ( noOfRows > maxNoOfRows )
    {
      ...
      if ( clipF )
      {
        ...
        tmp = EwShareString( EwConcatString( EwConcatString( EwLoadString( &_Const0006 ), 
        EwStringMiddle( res, inxF, len )), EwLoadString( &_Const0006 )));
      ...
      if ( clipL && ( inxL >= inxF ))
      {
        ...
        XString tmp2 = EwShareString( EwConcatString( EwConcatString( EwLoadString( 
          &_Const0006 ), EwStringMiddle( res, inxL, len )), EwLoadString( &_Const0006 )));
...
/* Default onget method for the property 'ColorBR' */
XColor ViewsText_OnGetColorBR( ViewsText _this )
{
  return _this->ColorBR;
}

/* Default onget method for the property 'ColorTL' */
XColor ViewsText_OnGetColorTL( ViewsText _this )
{
  return _this->ColorTL;
}

/* Default onget method for the property 'ScrollOffset' */
XPoint ViewsText_OnGetScrollOffset( ViewsText _this )
{
  return _this->ScrollOffset;
}
...
/* Initializer for the class 'Views::FilterScreen' */
void ViewsFilterScreen__Init( ViewsFilterScreen _this, XObject aLink, XHandle aArg )
{
  /* At first initialize the super class ... */
  CoreRectView__Init( &_this->_.Super, aLink, aArg );

  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_.XObject._.GCT = EW_CLASS_GCT( ViewsFilterScreen );

  /* Setup the VMT pointer */
  _this->_.VMT = EW_CLASS( ViewsFilterScreen );
...
/* Re-Initializer for the class 'Views::FilterScreen' */
void ViewsFilterScreen__ReInit( ViewsFilterScreen _this )
{
  /* At first re-initialize the super class ... */
  CoreRectView__ReInit( &_this->_.Super );
}

/* Finalizer method for the class 'Views::FilterScreen' */
void ViewsFilterScreen__Done( ViewsFilterScreen _this )
{
  /* Finalize this class */
  _this->_.Super._.VMT = EW_CLASS( CoreRectView );
  ...
  aClip, XPoint aOffset, XInt32 aOpacity, XBool aBlend )
{
  ...
  if ( _this->drawing )
  {
    aCanvas->stopDrawing = 1;
    return;
  }

  aBlend = (XBool)( aBlend && (( _this->Super2.viewState & CoreViewStateAlphaBlended ) 
  == CoreViewStateAlphaBlended ));
  aOpacity = ( 255 * ( aOpacity + 1 )) >> 8;

  if (( _this->BlurClampToEdges && ( aOpacity == 255 )) && ( _this->TintMode == 
      GraphicsTintModeNone ))
    aBlend = 0;

  blurRadius = ViewsFilterScreen_AdjustBlurRadius( _this, 0 );
  ...
  if (( aBlend && !!!_this->TintColor.Alpha ) && (( _this->TintMode == GraphicsTintModeTintByOpacity ) 
      || ( _this->TintMode == GraphicsTintModeTintByLuminance )))
    return;

  exposed = _this->Super1.Bounds;
  bounds = EwMoveRectPos( _this->Super1.Bounds, aOffset );
  offset = aOffset;
  group = _this->Super2.Owner;

  for ( ; group != 0; group = group->Super2.Owner )
  {
    exposed = EwIntersectRect( exposed, EwGetRectORect( group->Super1.Bounds ));

    if ( group == aCanvas->group )
      break;

    exposed = EwMoveRectPos( exposed, group->Super1.Bounds.Point1 );
    offset = EwMovePointNeg( offset, group->Super1.Bounds.Point1 );
  }

  render = bounds;
  render = EwIntersectRect( render, EwMoveRectPos( exposed, offset ));
  render = EwIntersectRect( render, EwInflateRect( aClip, EwNewPoint( blurRadius, 
  blurRadius )));

  if ( EwIsRectEmpty( render ) || EwIsRectEmpty( aClip ))
    return;

  canvas = EwNewObject( GraphicsCanvas, 0 );
  ResourcesBitmap__OnSetFrameSize( canvas, EwGetRectSize( render ));
  _this->drawing = 1;

  {
    XRect r = EwGetRectORect( render );
    canvas->group = aCanvas->group;
    GraphicsCanvas_FillRectangle( canvas, r, r, _Const0007, _Const0007, _Const0007, 
    _Const0007, 0 );
    CoreView__Draw( canvas->group, canvas, r, EwMovePointNeg( EwMovePointPos( EwNegPoint( 
    render.Point1 ), offset ), canvas->group->Super1.Bounds.Point1 ), 255, 1 );
    canvas->group = 0;
  }

  _this->drawing = 0;
  srcPos = EwMovePointNeg( EwMovePointPos( exposed.Point1, offset ), render.Point1 );
  maskPos = EwMovePointNeg( EwMovePointPos( exposed.Point1, offset ), bounds.Point1 );
  GraphicsCanvas_FilterBitmap( aCanvas, aClip, ((ResourcesBitmap)canvas ), 0, EwMoveRectPos( 
  exposed, offset ), srcPos, maskPos, 0, 0, blurRadius, _this->BlurClampToEdges, 
  _this->TintMode, 1.0f, _this->TintColor, aOpacity, aBlend );
  ResourcesBitmap__OnSetFrameSize( canvas, _Const0004 );
}
...
{
  if ( value == _this->BlurClampToEdges )
    return;

  _this->BlurClampToEdges = value;
}
...
/* Variants derived from the class : 'Views::FilterScreen' */
EW_DEFINE_CLASS_VARIANTS( ViewsFilterScreen )
EW_END_OF_CLASS_VARIANTS( ViewsFilterScreen )

/* Virtual Method Table (VMT) for the class : 'Views::FilterScreen' */
EW_DEFINE_CLASS( ViewsFilterScreen, CoreRectView, _.VMT, _.VMT, _.VMT, _.VMT, _.VMT, 
                 _.VMT, "Views::FilterScreen" )
  CoreRectView_initLayoutContext,
  CoreView_GetRoot,
  ViewsFilterScreen_Draw,
  CoreView_GetClipping,
  CoreView_HandleEvent,
  CoreView_CursorHitTest,
  ViewsFilterScreen_AdjustDrawingArea,
  CoreRectView_ArrangeView,
  CoreRectView_MoveView,
  CoreRectView_GetExtent,
  CoreView_ChangeViewState,
  CoreRectView_OnSetBounds,
EW_END_OF_CLASS( ViewsFilterScreen )
